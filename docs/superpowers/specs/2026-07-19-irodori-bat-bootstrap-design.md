...
**Scope:** `ParallelWorld_run.bat` から開発版Parallel Worldを起動する際の、Irodori-TTS環境確認・任意構築・起動・終了処理
...
5. Irodori serverをloopback限定で起動し、health、voice list、短文WAV warm-upを確認する。
6. Parallel Worldをforegroundで起動する。
7. Parallel World終了後、この起動セッションが開始したSTT/TTS関連processだけを停止する。外部起動済みTTSとLLMは残す。

## Managed Environment

Irodori環境はrepository内ではなく、OSのユーザーデータ領域に配置する。配置はversioned directoryとし、server source、portable `uv`、uv-managed Python 3.10、virtual environment、model、codec、download cache、transaction stateを分離する。
...
system Python、Git、repository-local virtual environmentは使用しない。portable `uv`、server archive、Python、dependency lock、model、codecは固定versionまたはrevisionとSHA-256をmanifestで管理する。

## Platform Backend Selection

- Windows/Linux + supported NVIDIA GPU: `cu128`
- Linux + supported Radeon GPU and ROCm runtime: `rocm`
- Windows + Radeon: `cpu`; WSL/ROCm support is deferred
- Apple Silicon: standard macOS PyTorch wheel with `mps`
- Intel Mac and unsupported hardware: `cpu`
...
bootstrap sessionは、自身が起動したprocessのPIDとownership markerを保持する。

- Irodori、bootstrap中の`uv`/Python、アプリ管理のAivisSpeech、外部STT sidecarはprocess tree単位で停止する。
...
環境検査、download、構築、server起動、warm-upの失敗はアプリ全体を停止させない。安全な短いエラーを表示してTTSを縮退させ、Parallel Worldの起動を続行する。credential、authorization header、環境変数全体、ユーザーファイルの完全pathはログへ出力しない。
...
- health、voice list、RIFF/WAVE warm-up判定
- owned STT/TTSだけの停止と外部TTS/LLMの維持
- LoRAと参照音声の保持
- secretとユーザーpathのlog redaction

実serviceを使うacceptance testは明示的な環境変数opt-inとし、通常のtest suiteでは実model、外部API、有料serviceへ接続しない。公開前には対応platformのclean environmentで、確認、構築、warm-up、音声生成、終了後のprocess解放を確認する。
