...
長時間運転で資源が「一度も増えない」ことは要求しない。先頭2分をwarm-upとして除外し、残りの標本に線形回帰を適用する。RSSおよびprivate bytesは傾き64 MiB/hour以下、かつ最初と最後の増加64 MiB以下を合格条件とする。queue depthは各64以下、cacheは1,000 files以下、logは256 MiB以下とする。unexpected exit、panic、orphan processは0件でなければならない。
...
heartbeatはUTF-8 JSONで、次のschema version 1をatomic replaceで定期更新する。`timestamp_ms`、`audio_device`、queue、drop、cache、log、restart、panic、fault、監視対象child PIDを実測値として書く。
...
fault injectionは `-FaultInjection -FaultTarget OwnedChild -ConfirmOwnedFault` の3点を明示した場合だけ、harnessが起動したrootの子孫を対象にする。任意PIDや外部serviceは停止しない。cleanupも観測時のPIDとprocess開始時刻が一致するprocessだけを回収し、長時間run中に再利用されたPIDを停止しない。kill後10秒deadlineをtimelineへ記録し、別PIDの子processが観測されなければ失敗する。外部AivisSpeech/llama-serverのfault試験はこのflagでは行わず、個別の手動確認手順を用いる。
